...
// NewArrayList ,init ArrayList with input type and capacity
func NewArrayList(typeOf string, capacity int) (*ArrayList, error) {
	if typeOf == "" {
...
func (arr *ArrayList) Add(val interface{}) error {
	if err := arr.checkType(val); err != nil {
		return err
	}
...
func (arr *ArrayList) checkType(val interface{}) error {
...
func (arr *ArrayList) SubArrayList(start, end int) (*ArrayList, error) {
	if start > end {
		return nil, errors.SubArrayListWithIndexError
	}
